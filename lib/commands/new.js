...
var repositories = {
  jam: 'https://github.com/zizther/jam.git',
  jamvue: 'https://github.com/zizther/jam-vue.git'
}

module.exports = function(args, options, callback, ee) {
  var projectName, projectFolder, webroot, vue, vueroutes, messages, directory;
  var tasks = [
  ...
  function prompt(cb) {
    inquirer.prompt(util.questions(options), function(answers) {
      ...
      webroot = answers.webroot || options.webroot;
      vue = answers.vue || options.vue;
      vueroutes = answers.vueroutes || options.vueroutes;
  ...
  function gitClone(cb) {
    var repo = repositories['jam'];

    var cmd = format('git clone %s %s', repo, projectName);
    var hello = formatHello(messages.helloYeti, 'Jam');

    console.log(util.mascot('jam', hello));
